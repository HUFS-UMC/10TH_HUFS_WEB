...
const LpDetailPage = () => {
  ...
  const { mutate: toggleLikeMutation } = useMutation({
    mutationFn: async () => {
      const isCurrentlyLiked = lpData?.likes?.some((like: any) => like.userId === myUserId) || false;
      if (isCurrentlyLiked) {
    ...
    onMutate: async () => {
      ...
      const previousLpData = queryClient.getQueryData(["lpDetail", lpId]);
    ...
    onError: (error, variables, context) => {
      if (context?.previousLpData) {
        queryClient.setQueryData(["lpDetail", lpId], context.previousLpData);
      }
    },
    onSettled: () => {
      queryClient.invalidateQueries({ queryKey: ["lpDetail", lpId] });
    ...
    <div className="max-w-2xl mx-auto p-6 bg-zinc-900 rounded-3xl mt-10 shadow-xl border border-zinc-800">
      ...
      <div className="flex items-center justify-between mb-6">
        <div className="flex items-center gap-3">
          <img src={lpData.author?.avatar || "/default-avatar.png"} className="w-10 h-10 rounded-full object-cover" alt="author avatar" />
          <div>
      ...
      <div className="space-y-4">
        ...
        <div className="relative rounded-2xl overflow-hidden bg-black aspect-square">
          <img 
            src={preview || lpData.thumbnail || "https://placehold.co/500x500?text=No+Image"} 

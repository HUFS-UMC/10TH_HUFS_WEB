...
// 응답 인터셉터: 401 에러 발생 시 refresh token으로 access token을 갱신한다.
axiosInstance.interceptors.response.use(
  ...
  async (error: AxiosError) => {
    ...
    if (
      ...
      // refresh 요청 자체에서 401이 발생한 경우, 무한 재시도 방지를 위해 로그아웃 처리
      ...
      // 이미 refresh 요청이 진행 중이면 그 Promise를 재사용한다.
      if (!refreshPromise) {
        refreshPromise = (async () => {
          ...
          // 새 accessToken을 반환해서 다른 요청들이 이것을 사용할 수 있게 함

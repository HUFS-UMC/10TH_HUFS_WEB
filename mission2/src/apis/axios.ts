...
interface CustomInteralAxiosRequestConfig extends InternalAxiosRequestConfig {
  _retry?:boolean; // 요청 재시도 여부를 나타내는 플래그 (재시도 여부 안 물어보면 무한히 반복하니깐 그걸 방지하기 위해)
}
...
// 응답 인터셉터: 401 에러발생시 refreshToken을 통한 토큰 갱신을 처리한다.
axiosInstance.interceptors.response.use(
  ...
  async(error) => {
    ...
    if (
      ...
      // 아무때나 요청이 일어나면 안됨 refresh end point에서 401에러가 발생한 경우, 중복 재시도 방지를 위해 로그아웃 처리.
      if (originalRequest.url === "/v1/auth/refresh") {
        ...
        removeAccessToken();
        removeRefreshToken();
        window.location.href = "/login"; // 토큰을 다 빼주면 로그인 페이지로 옮기면서 에러도 띄워줌
